...
# 本文件先采取dlib库中的cnn模型检测人脸是否存在，若不存在，再使用hog模型检测，还不存在的话就忽略人脸检测
...
for f in islice(index_txt, start + 1257, start + 4314):
    f = f.strip('\n')
    ...
    if len(dets2) == 0:
        ...
        if len(dets) == 0:
            ...
            shape = sp(img, rec)
            count2 += 1
            print("Number of faces detected to ignore: {}".format(len(dets2)))
        else:
            print("Number of faces detected by hog: {}".format(len(dets)))
